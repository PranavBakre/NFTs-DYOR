...
let model = (document.getElementById("model").innerHTML =
  params.get("model") ?? "avatar.glb");
let logo = (document.getElementById("logo").innerHTML =
  params.get("logo") ?? "logo.png");
let title = (document.getElementById("title").innerHTML =
  params.get("title") ?? "Megha Singh");
let subtitle = (document.getElementById("subtitle").innerHTML =
  params.get("subtitle") ?? "Member since Jan 2021");
...
let titleFont = new FontFace(
  "Gilroy-Medium",
  "url(https://fonts.cdnfonts.com/s/16219/Gilroy-Medium.woff)"
);
let subtitleFont = new FontFace(
  "Gilroy-Regular",
  "url(https://fonts.cdnfonts.com/s/16219/Gilroy-Regular.woff)"
);

Promise.all([titleFont.load(), subtitleFont.load()])
  .then((fonts) => {
    console.log(fonts);
    fonts.forEach((font) => document.fonts.add(font));
  })
  .then(() => {
    let { texture: canvasTexture, canvas: titleCanvas } = convertTextToTexture(
      title,
      {
        position: "absolute",
        top: "0px",
    ...
    let titleGeometry = new PlaneBufferGeometry(
    ...
    let { texture: subtitleCanvasTexture, canvas: subtitleCanvas } =
    ...
    let subtitleMaterial = new MeshBasicMaterial({
      map: subtitleCanvasTexture,
      transparent: true,
    });
    let subtitleGeometry = new PlaneBufferGeometry(
      subtitleCanvas.width / 500,
      subtitleCanvas.height / 450,
      2,
      2
    );
...
const frameBorder1Mesh = new Mesh(
  new ShapeBufferGeometry(frameBorder1),
  borderMaterial
);
...
y = y + borderSize + 0.05;
width = width - 2 * borderSize;
height = height - 2 * borderSize - 0.05;
radius = radius - 0.005;
...
y = y + borderSize;
width = width - 2 * borderSize;
height = height - 2 * borderSize; //radius = radius - 0.005;
const frameBorder2Mesh = new Mesh(
  new ShapeBufferGeometry(frameBorder2),
  borderMaterial
);
